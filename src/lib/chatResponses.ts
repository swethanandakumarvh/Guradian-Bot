...
type ResponseKey = 'greeting' | 'emergency.confirmation' | 'mental_support.calming' | 'emergency.type' | 'emergency.location' | 'emergency.contact';
...
export function getResponse(key: ResponseKey, language: LanguageCode = 'en', params: Record<string, string> = {}): string {
  try {
    let response = '';
    if (key.includes('.')) {
      const [category, subKey] = key.split('.');
      response = chatResponses[category][subKey][language];
    } else {
      response = chatResponses[key][language];
    }
